...
def main():
    ...
    with torch.no_grad():
        while True:
            ...
            if args.record and n_frames == 1000:
                env.stop_recording(
                    save_path_behind=f"{args.exp_name}_{args.ckpt}_{args.vel_x}_behind.mp4",
                    save_path_side=f"{args.exp_name}_{args.ckpt}_{args.vel_x}_side.mp4",
                )
    ...
    ax1.grid(True)
    fig1.tight_layout()
    fig1.savefig(f"speed_plot_{args.ckpt}_{args.vel_x}.png")
    plt.close(fig1)
    ...
    ax2.grid(True)
    fig2.tight_layout()
    fig2.savefig(f"ang_vel_plot_{args.ckpt}_{args.vel_x}.png")
    plt.close(fig2)
    ...
    fig3.tight_layout()
    fig3.savefig(f"trajectory_plot_{args.ckpt}_{args.vel_x}.png")
    plt.close(fig3)
