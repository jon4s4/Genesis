...
from reward_wrapper import RunCurveTerrain
...
def main():
    ...
    env_cfg["terrain_cfg"]["subterrain_types"] = [
                ['fractal_terrain', 'fractal_terrain', 'fractal_terrain'],
                ['fractal_terrain', 'fractal_terrain', 'fractal_terrain'],
                ['fractal_terrain', 'fractal_terrain', 'fractal_terrain']
            ]
    ...
    with torch.no_grad():
        while True:
            ...
            if args.record and n_frames == 1000:
                env.stop_recording(
                    save_path_behind=f"{args.exp_name}_fractal_{args.ckpt}_{args.vel_x}_{args.ang_vel}_behind.mp4",
                    save_path_side=f"{args.exp_name}_fractal_{args.ckpt}_{args.vel_x}_{args.ang_vel}_side.mp4",
                )
    ...
    ax1.grid(True)
    fig1.tight_layout()
    fig1.savefig(f"speed_plot_fractal_{args.ckpt}_{args.vel_x}_{args.ang_vel}.png")
    plt.close(fig1)
    ...
    ax2.grid(True)
    fig2.tight_layout()
    fig2.savefig(f"ang_vel_plot_fractal_{args.ckpt}_{args.vel_x}_{args.ang_vel}.png")
    plt.close(fig2)
    ...
    fig3.tight_layout()
    fig3.savefig(f"trajectory_plot_fractal_{args.ckpt}_{args.vel_x}_{args.ang_vel}.png")
    plt.close(fig3)
