...
class Go2Env:
    ...
    def _add_terrain(self):
        ...
        if self.use_terrain:
            ...
            self.base_init_pos = torch.tensor(
                self.env_cfg.get("base_init_pos", [0.0, 0.0, 0.35]),
                device=self.device
            )
    ...
    def _add_obstacles(self):
        """Generiert Slalomstangen und speichert deren Positionen als Tensor."""
        pole_list = []
        
        # Konfiguration für den Slalom
        start_x = 2.0        # Erste Stange bei x = 2m
        spacing_x = 2.0      # Abstand zwischen den Stangen in Laufrichtung
        offset_y = 0.5       # Wie weit die Stangen nach links/rechts versetzt sind
        num_poles = 8        # Anzahl der Stangen insgesamt
        
        # Generiere die Koordinaten abwechselnd links und rechts
        for i in range(num_poles):
            x = start_x + (i * spacing_x)
            # Abwechselnd +0.5 und -0.5
            y = offset_y if i % 2 == 0 else -offset_y 
            
            pole_list.append([x, y])
            
            # Füge die physische Stange direkt zur Genesis-Szene hinzu
            self.scene.add_entity(
                gs.morphs.Cylinder(
                    radius=0.05,        # 5cm Radius
                    height=1.0,         # 1m hoch
                    pos=(x, y, 0.5),    # Z=0.5, da der Ursprung in der Mitte des Zylinders liegt
                    fixed=True          # True = unbewegliches Hindernis
                )
            )

        # Speichere die Positionen als PyTorch Tensor auf der GPU für schnelle Reward/Obs-Berechnung
        self.pole_positions = torch.tensor(
            pole_list, 
            device=self.device, 
            dtype=gs.tc_float
        )
    ...
    def _check_termination(self):
        ...
        # SAFETY CHECK: Falls NaNs in der Basis-Position oder Lineargeschwindigkeit auftreten
        nan_mask = torch.isnan(self.base_pos).any(dim=-1) | torch.isnan(self.base_lin_vel).any(dim=-1)
        self.reset_buf |= nan_mask # Erzwinge Reset für kaputte Umgebungen
    ...
    def _compute_observations(self):
        ...
        # --- NEU: Berechnung des relativen 2D-Vektors zur nächsten Slalomstange ---
        # Berechne die Distanz in X-Richtung zu allen Stangen für jede Umgebung
        # self.pole_positions hat das Shape (num_poles, 2) -> [[x1, y1], [x2, y2], ...]
        dx = self.pole_positions[:, 0] - self.base_pos[:, 0].unsqueeze(1) # Shape: (num_envs, num_poles)
        
        # Klone dx, um den originalen Zustand nicht zu überschreiben
        dx_masked = dx.clone()
        # Maskiere Stangen, die der Roboter bereits hinter sich gelassen hat (dx < 0)
        dx_masked[dx_masked < 0.0] = 999.0 
        
        # Finde den Index der am nächsten vor dem Roboter stehenden Stange
        next_pole_idx = torch.argmin(dx_masked, dim=1) # Shape: (num_envs,)
        
        # Hole die absolute 2D-Position (X, Y) dieser Stange
        next_pole_pos = self.pole_positions[next_pole_idx] # Shape: (num_envs, 2)
        
        # Berechne den relativen Vektor (X, Y) vom Roboter zur Stange
        relative_pole_pos = next_pole_pos - self.base_pos[:, :2] # Shape: (num_envs, 2)
        # ------------------------------------------------------------------------
        ...
        obs = [ 
                ...
                relative_pole_pos, # <--- NEU: Fügt die 2 fehlenden Dimensionen hinzu (51 + 2 = 53)
        ]
        if self.use_terrain:
            obs.append(self.relative_heights) # 25 -> Gesamt: 78
        ...
        if self.num_privileged_obs is not None:
            ...
            privileged_obs = [
                ...
                self.base_pos - self.last_base_pos, # 3
                relative_pole_pos, # <--- NEU: Auch hier einfügen (75 + 2 = 77)
            ]
            if self.use_terrain:
                privileged_obs.append(self.relative_heights) # 25 -> Gesamt: 102
