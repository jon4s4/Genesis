...
import wandb
from reward_wrapper import SlalomObstacleTerrain
...
def get_cfgs():
    ...
    reward_cfg = {
        "tracking_sigma": 0.25, # Kleinerer Wert macht das Tracking präziser
        "reward_scales": {
            # Vorwärtsbewegung belohnen
            "tracking_lin_vel_x": 1.5,
            "slalom_progress": 2.0,        # Leicht gesenkt für mehr Fokus auf die Spur
            
            # Das Ausbrechen verhindern (NEU)
            "corridor_boundary": -3.0,     # Starke Strafe fürs Verlassen der Gasse
            
            # Hindernis-Vermeidung
            "obstacle_collision": -1.5,    
            
            # Stabilität (unverändert)
            "lin_vel_z": -1.0,
            "lin_vel_y": -0.05,            
            "action_rate": -0.01,
    ...
    command_cfg = {
        "num_commands": 3,
        "lin_vel_x_range": [1.0, 2.5],   # Nutze eine Range für dynamischere Fortbewegung
        "lin_vel_y_range": [-0.0, 0.0],  
        "ang_vel_range": [-0.0, 0.0],    # Ang_vel Kommando bleibt 0, die Drehung kommt rein aus der Obstacle Avoidance
    }
